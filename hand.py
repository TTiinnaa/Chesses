...
class Hand(object):
	...
	def update(self,key_down):
			...
			if key_down== self.con["up"]:
				newy=self.y-self.sp


			elif key_down == self.con["down"]:
				newy=self.y+self.sp

			if key_down== self.con["left"]:
				newx=self.x-self.sp

			elif key_down == self.con["right"]:
				newx=self.x+self.sp


			if key_down == self.con["counter_c"]:
				self.angle+=10
				#print(self.angle)
			if key_down == self.con["clock"]:
				self.angle-=10
			...
			if self.game.is_pick():
				self.handle_picking(key_down)
			else:
				self.handle_shooting(key_down)
	...
	def handle_shooting(self,key_down):
			if key_down==self.con["pick"]:
				self.shoot=True
				self.pick=True
	...
	def __setattr__(self,k,v):
		...
		super(Hand, self).__setattr__(k,v)
